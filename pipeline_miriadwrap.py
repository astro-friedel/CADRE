...
def coinit(lu) :
    ...
    luX = miriad_io.copy_intp(lu)
    try :
        XX = mx.safecall(miriad_io.coinit_,(luX,))
        del XX
    except :
        pass
...
def codup(lin) :
    ...
    """
    linX = miriad_io.copy_intp(lin)
    loutX = miriad_io.new_intp()
    try :
        XX = mx.safecall(miriad_io.codup_,(linX,loutX))
        del XX
    except :
        pass
    loutR = miriad_io.intp_value(loutX)
    return loutR

def coradec(proj,ra0,dec0) :
    """ Miriad wrapper - see miriad documentation for full description
        input :
            proj -
            ra0 -
    ...
    ra0X = miriad_io.copy_doublep(ra0)
    dec0X = miriad_io.copy_doublep(dec0)
    try :
        XX = mx.safecall(miriad_io.coradec_,(luX,proj,ra0X,dec0X,len(proj)))
        del XX
    except :
        pass
...
def cocreate() :
    ...
    luX = miriad_io.new_intp()
    try :
        XX = mx.safecall(miriad_io.cocreate_,(luX,))
        del XX
    except :
        pass
    luR = miriad_io.intp_value(luX)
...
def coaxset(lu,iax,ctypei,crpixi,crvali,cdelti) :
    """ Miriad wrapper - see miriad documentation for full description
        input :
            lu -
            iax -
    ...
    crpixiX = miriad_io.copy_doublep(crpixi)
    crvaliX = miriad_io.copy_doublep(crvali)
    cdeltiX = miriad_io.copy_doublep(cdelti)
    try :
        XX = mx.safecall(miriad_io.coaxset_,(luX,iaxX,ctypei,crpixiX,crvaliX,cdeltiX,len(ctypei)))
        del XX
    except :
        pass

def cosetd(lu,objt,value) :
    """ Miriad wrapper - see miriad documentation for full description
        input :
            lu -
            objt -
    ...
    try :
        XX = mx.safecall(miriad_io.cosetd_,(luX,objt,valueX,len(objt)))
        del XX
    except :
        pass

def coseta(lu,objt,value) :
    """ Miriad wrapper - see miriad documentation for full description
        input :
            lu -
            objt -
    ...
    luX = miriad_io.copy_intp(lu)
    try :
        XX = mx.safecall(miriad_io.coseta_,(luX,objt,value,len(objt),len(value)))
        del XX
    except :
        pass
...
def cogetd(lu,objt) :
    ...
    valueX = miriad_io.new_doublep()
    try :
        XX = mx.safecall(miriad_io.cogetd_,(luX,objt,valueX,len(objt)))
        del XX
    except :
        pass
    valueR = miriad_io.doublep_value(valueX)
...
def cogeta(lu,objt) :
    ...
    luX = miriad_io.copy_intp(lu)
    try :
        XX = mx.safecall(miriad_io.cogeta_,(luX,objt,value,len(objt),20))
        del XX
    except :
        pass
    value = value.strip()
...
def coreinit(lu) :
    ...
    luX = miriad_io.copy_intp(lu)
    try :
        XX = mx.safecall(miriad_io.coreinit_,(luX,))
        del XX
    except :
        pass
...
def cofin(lu) :
    ...
    luX = miriad_io.copy_intp(lu)
    try :
        XX = mx.safecall(miriad_io.cofin_,(luX,))
        del XX
    except :
        pass

def cocvtv(lu,inp,x1,out) :
    """ Miriad wrapper - see miriad documentation for full description
        input :
            lu -
            inp -
    ...
    """
    luX = miriad_io.copy_intp(lu)
    x1X = doubleListToArray(x1)
    l = len(x1)
    x2X = doubleArray(l)
    validX = miriad_io.new_intp()
    try :
        XX = mx.safecall(miriad_io.cocvtv_,(luX,inp,x1X,out,x2X,validX,len(inp),len(out)))
        del XX
    except :
        pass
    x2R = doubleArrayToList(x2X,l)
    del x2X
    validR = castLogical(miriad_io.intp_value(validX))
    return x2R,validR

def cofreq(lu,inp,x1) :
    """ Miriad wrapper - see miriad documentation for full description
        input :
            lu -
            in -
    ...
    luX = miriad_io.copy_intp(lu)
    x1X = doubleListToArray(x1)
    freq1X = miriad_io.new_doublep()
    try :
        XX = mx.safecall(miriad_io.cofreq_,(luX,inp,x1X,freq1X,len(inp)))
        del XX
    except :
        pass
    freq1R = miriad_io.doublep_value(freq1X)
    return freq1R
...
def colmn(lu,inp,x1) :
    ...
    lmnX = doubleArray(3)
    try :
        XX = mx.safecall(miriad_io.colmn_,(luX,inp,x1X,lmnX,len(inp)))
        del XX
    except :
        pass
...
def cogeom(lu,inp,x1) :
    ...
    ucoeffX = doubleArray(3)
    vcoeffX = doubleArray(3)
    wcoeffX = doubleArray(3)
    try :
        XX = mx.safecall(miriad_io.cogeom_,(luX,inp,x1X,ucoeffX,vcoeffX,wcoeffX,len(inp)))
        del XX
    except :
        pass
    ucoeffR = doubleArrayToList(ucoeffX,3)
    del ucoeffX
    vcoeffR = doubleArrayToList(vcoeffX,3)
    del vcoeffX
    wcoeffR = doubleArrayToList(wcoeffX,3)
    del wcoeffX
    return ucoeffR,vcoeffR,wcoeffR

def cocvt1(lu,iax,inp,x1,out) :
    """ Miriad wrapper - see miriad documentation for full description
        input :
            lu -
            iax -
    ...
    iaxX = miriad_io.copy_intp(iax)
    x1X = miriad_io.copy_doublep(x1)
    x2X = miriad_io.new_doublep()
    try :
        XX = mx.safecall(miriad_io.cocvt1_,(luX,iaxX,inp,x1X,out,x2X,len(inp),len(out)))
        del XX
    except :
        pass
    x2R = miriad_io.doublep_value(x2X)
    return x2R

def cogaucvt(lu,inp,x1,ing,bmaj1,bmin1,bpa1,outg) :
    """ Miriad wrapper - see miriad documentation for full description
        input :
            lu -
            inp -
    ...
    bmaj2X = miriad_io.new_floatp()
    bmin2X = miriad_io.new_floatp()
    bpa2X = miriad_io.new_floatp()
    try :
        XX = mx.safecall(miriad_io.cogaucvt_,(luX,inp,x1X,ing,bmaj1X,bmin1X,bpa1X,outg,bmaj2X,bmin2X,bpa2X,len(inp),len(ing),len(outg)))
        del XX
    except :
        pass
    bmaj2R = miriad_io.floatp_value(bmaj2X)
    bmin2R = miriad_io.floatp_value(bmin2X)
    bpa2R = miriad_io.floatp_value(bpa2X)
    return bmaj2R,bmin2R,bpa2R

def covelset(lu,typeX) :
    """ Miriad wrapper - see miriad documentation for full description
        input :
            lu -
            typeX -
    ...
    luX = miriad_io.copy_intp(lu)
    try :
        XX = mx.safecall(miriad_io.covelset_,(luX,typeX,len(typeX)))
        del XX
    except :
        pass
...
def coaxget(lu,iax) :
    ...
    luX = miriad_io.copy_intp(lu)
    iaxX = miriad_io.copy_intp(iax)
    crpixiX = miriad_io.new_doublep()
    crvaliX = miriad_io.new_doublep()
    cdeltiX = miriad_io.new_doublep()
    try :
        XX = mx.safecall(miriad_io.coaxget_,(luX,iaxX,ctypei,crpixiX,crvaliX,cdeltiX,20))
        del XX
    except :
        pass
    crpixiR = miriad_io.doublep_value(crpixiX)
    crvaliR = miriad_io.doublep_value(crvaliX)
    cdeltiR = miriad_io.doublep_value(cdeltiX)
    ctypei = ctypei.strip()
    return ctypei,crpixiR,crvaliR,cdeltiR

def cofindax(lu,axis) :
    """ Miriad wrapper - see miriad documentation for full description
        input :
            lu -
            axis -
    ...
    """
    luX = miriad_io.copy_intp(lu)
    iaxX = miriad_io.new_intp()
    try :
        XX = mx.safecall(miriad_io.cofindax_,(luX,axis,iaxX,len(axis)))
        del XX
    except :
        pass
    iaxR = miriad_io.intp_value(iaxX)
    return iaxR

def cocompar(lu1,lu2,match) :
    """ Miriad wrapper - see miriad documentation for full description
        input :
            lu1 -
            lu2 -
        ...
        returns :   (as a tuple)
            cocompar -
    """
    lu1X = miriad_io.copy_intp(lu1)
    lu2X = miriad_io.copy_intp(lu2)
    XX = None
    try :
        XX = mx.safecall(miriad_io.cocompar_,(lu1X,lu2X,match,len(match)))
    except :
        pass
    return castLogical(XX)
...
def coprint(lu) :
    ...
    luX = miriad_io.copy_intp(lu)
    try :
        XX = mx.safecall(miriad_io.coprint_,(luX,))
        del XX
    except :
        pass
...
def coprjset(lu,p) :
    ...
    luX = miriad_io.copy_intp(lu)
    try :
        XX = mx.safecall(miriad_io.coprjset_,(luX,p,len(p)))
        del XX
    except :
        pass
...
def cowrite(lu,tno) :
    ...
    tnoX = miriad_io.copy_intp(tno)
    try :
        XX = mx.safecall(miriad_io.cowrite_,(luX,tnoX))
        del XX
    except :
        pass

def cocrack(code,defn,maxnax) :
    """ Miriad wrapper - see miriad documentation for full description
        input :
            code -
            defn -
        ...
        returns :   (as a tuple)
            ...
            n -
    """
    x1pixX = intArray(maxnax)
    x1offX = intArray(maxnax)
    defnX = miriad_io.copy_intp(defn)
    maxnaxX = miriad_io.copy_intp(maxnax)
    nX = miriad_io.new_intp()
    try :
        XX = mx.safecall(miriad_io.cocrack_,(code,x1pixX,x1offX,defnX,maxnaxX,nX,len(code)))
        del XX
    except :
        pass
    nR = miriad_io.intp_value(nX)
    x1pixR = castLogical(intArrayToList(x1pixX,nR))
    del x1pixX
    x1offR = castLogical(intArrayToList(x1offX,nR))
    del x1offX
    return x1pixR,x1offR

def cofelo(x10,xval,xpix,dx,x1pix,x1off,x2pix,x2off) :
    """ Miriad wrapper - see miriad documentation for full description
        input :
            x10 -
            xval -
        ...
        returns :   (as a tuple)
            x2 -
    """
    x10X = miriad_io.copy_doublep(x10)
    x2X = miriad_io.new_doublep()
    xvalX = miriad_io.copy_doublep(xval)
    xpixX = miriad_io.copy_doublep(xpix)
    dxX = miriad_io.copy_doublep(dx)
    x1pixX = miriad_io.copy_intp(uncastLogical(x1pix))
    x1offX = miriad_io.copy_intp(uncastLogical(x1off))
    x2pixX = miriad_io.copy_intp(uncastLogical(x2pix))
    x2offX = miriad_io.copy_intp(uncastLogical(x2off))
    try :
        XX = mx.safecall(miriad_io.cofelo_,(x10X,x2X,xvalX,xpixX,dxX,x1pixX,x1offX,x2pixX,x2offX))
        del XX
    except :
        pass
    x2R = miriad_io.doublep_value(x2X)
...
def cocelest(x10,y10,proj,llcos,llsin,xval,xpix,dx,yval,ypix,dy,x1pix,y1pix,x2pix,y2pix,x1off,y1off,x2off,y2off) :
    """ Miriad wrapper - see miriad documentation for full description
        input :
            x10 -
            y10 -
    ...
    """
    x10X = miriad_io.copy_doublep(x10)
    y10X = miriad_io.copy_doublep(y10)
    x2X = miriad_io.new_doublep()
    y2X = miriad_io.new_doublep()
    llcosX = miriad_io.copy_doublep(llcos)
    llsinX = miriad_io.copy_doublep(llsin)
    xvalX = miriad_io.copy_doublep(xval)
    xpixX = miriad_io.copy_doublep(xpix)
    dxX = miriad_io.copy_doublep(dx)
    yvalX = miriad_io.copy_doublep(yval)
    ypixX = miriad_io.copy_doublep(ypix)
    dyX = miriad_io.copy_doublep(dy)
    x1pixX = miriad_io.copy_intp(uncastLogical(x1pix))
    y1pixX = miriad_io.copy_intp(uncastLogical(y1pix))
    x2pixX = miriad_io.copy_intp(uncastLogical(x2pix))
    y2pixX = miriad_io.copy_intp(uncastLogical(y2pix))
    x1offX = miriad_io.copy_intp(uncastLogical(x1off))
    y1offX = miriad_io.copy_intp(uncastLogical(y1off))
    x2offX = miriad_io.copy_intp(uncastLogical(x2off))
    y2offX = miriad_io.copy_intp(uncastLogical(y2off))
    validX = miriad_io.new_intp()
    try :
        XX = mx.safecall(miriad_io.cocelest_,(x10X,y10X,x2X,y2X,proj,llcosX,llsinX,xvalX,xpixX,dxX,yvalX,ypixX,dyX,x1pixX,y1pixX,x2pixX,y2pixX,x1offX,y1offX,x2offX,y2offX,validX,len(proj)))
        del XX
    except :
        pass
    x2R = miriad_io.doublep_value(x2X)
    y2R = miriad_io.doublep_value(y2X)
    validR = castLogical(miriad_io.intp_value(validX))
    return x2R,y2R,proj,proj,proj,validR

def colinear(xval,xpix,dx,x1pix,x1off,x2pix,x2off) :
    """ Miriad wrapper - see miriad documentation for full description
        input :
            xval -
            xpix -
    ...
    x2pixX = miriad_io.copy_intp(uncastLogical(x2pix))
    x2offX = miriad_io.copy_intp(uncastLogical(x2off))
    bscalX = miriad_io.new_doublep()
    bzeroX = miriad_io.new_doublep()
    try :
        XX = mx.safecall(miriad_io.colinear_,(xvalX,xpixX,dxX,x1pixX,x1offX,x2pixX,x2offX,bscalX,bzeroX))
        del XX
    except :
        pass
    bscalR = miriad_io.doublep_value(bscalX)
    bzeroR = miriad_io.doublep_value(bzeroX)
    return bscalR,bzeroR

def convlini(lu,n1,n2,phat,ic,jc) :
    """ Miriad wrapper - see miriad documentation for full description
        input :
            lu -
            n1 -
    ...
    """
    luX = miriad_io.copy_intp(lu)
    outX = floatArray(n2,n1/2 + 1)
    n1X = miriad_io.copy_intp(n1)
    n2X = miriad_io.copy_intp(n2)
    phatX = miriad_io.copy_floatp(phat)
    icX = miriad_io.copy_intp(ic)
    jcX = miriad_io.copy_intp(jc)
    try :
        XX = mx.safecall(miriad_io.convlini_,(luX,outX,n1X,n2X,phatX,icX,jcX))
        del XX
    except :
        pass
    outR = floatArrayToList(outX,n2,n1/2 + 1)
    del outX
    return outR
...
def coclose(lin) :
    ...
    linX = miriad_io.copy_intp(lin)
    try :
        XX = mx.safecall(miriad_io.coclose_,(linX,))
        del XX
    except :
        pass
...
def axfndco(lun,typeX,n,iax) :
    ...
    nX = miriad_io.copy_intp(n)
    iaxX = miriad_io.copy_intp(iax)
    jaxX = miriad_io.new_intp()
    try :
        XX = mx.safecall(miriad_io.axfndco_,(lunX,typeX,nX,iaxX,jaxX,len(typeX)))
        del XX
    except :
        pass
    jaxR = miriad_io.intp_value(jaxX)
    return jaxR

def axtypco(lun,n,iax) :
    """ Miriad wrapper - see miriad documentation for full description
        input :
            lun -
            n -
    ...
    iaxX = miriad_io.copy_intp(iax)
    try :
        XX = mx.safecall(miriad_io.axtypco_,(lunX,nX,iaxX,typeX,4))
        del XX
    except :
        pass
    typeX = typeX.strip()
...
def ctypeco(lun,iax) :
    ...
    lunX = miriad_io.copy_intp(lun)
    iaxX = miriad_io.copy_intp(iax)
    ilX = miriad_io.new_intp()
    try :
        XX = mx.safecall(miriad_io.ctypeco_,(lunX,iaxX,ctype,ilX,20))
        del XX
    except :
        pass
    ilR = miriad_io.intp_value(ilX)
    ctype = ctype.strip()
    return ctype,ilR
...
def finco(lun) :
    ...
    lunX = miriad_io.copy_intp(lun)
    try :
        XX = mx.safecall(miriad_io.finco_,(lunX,))
        del XX
    except :
        pass
...
def initco(lun) :
    ...
    lunX = miriad_io.copy_intp(lun)
    try :
        XX = mx.safecall(miriad_io.initco_,(lunX,))
        del XX
    except :
        pass
...
def uvgetvra(tno,var) :
    ...
    data = "                                                                                                    "
    try :
        mx.safecall(miriad_io.uvgetvra_,(t,var,data,len(var),98))
    except :
        pass
    data = data.strip()
    return data
...
def moduvdatrew() :
    ...
    XX = None
    try :
        XX = mx.safecall(miriad_io.moduvdatrew_)
    except :
        pass
    return XX
...
def moduvdatcls() :
    ...
    XX = None
    try :
        XX = mx.safecall(miriad_io.moduvdatcls_)
    except :
        pass
    return XX
...
def moduvpolget(n) :
    ...
    nreadX = miriad_io.new_intp()
    try :
        XX = mx.safecall(miriad_io.moduvpolget_,(preambleX,dataX,flagsX,nX,nreadX))
        del XX
    except :
        pass
    preambleR = doubleArrayToList(preambleX,12)
...
def moduvdatwrd(n) :
    ...
    nreadX = miriad_io.new_intp()
    try :
        XX = mx.safecall(miriad_io.moduvdatwrd_,(dataX,flagsX,nX,nreadX))
        del XX
    except :
        pass
    nreadR = miriad_io.intp_value(nreadX)
...
def moduvdatgti(objt,length = 100) :
    """ Miriad wrapper - see miriad documentation for full description
        input :
            objt -
        returns :   (as a tuple)
    ...
    if(length > 1) :
        ...
        ivalX = miriad_io.new_intp()
    try :
        XX = mx.safecall(miriad_io.moduvdatgti_,(objt,ivalX,len(objt)))
        del XX
    except :
        pass
    ivalR = None
...
def moduvdatgtr(objt) :
    ...
    rvalX = miriad_io.new_floatp()
    try :
        XX = mx.safecall(miriad_io.moduvdatgtr_,(objt,rvalX,len(objt)))
        del XX
    except :
        pass
    rvalR = miriad_io.floatp_value(rvalX)
...
def moduvdatgta(objt) :
    ...
    aval = "                           "
    try :
        XX = mx.safecall(miriad_io.moduvdatgta_,(objt,aval,len(objt),20))
        del XX
    except :
        pass
    aval = aval.strip()
...
def moduvlkini() :
    ...
    XX = None
    try :
        XX = mx.safecall(miriad_io.moduvlkini_)
    except :
        pass
    return XX

def moduvlkcorr(baseline,maxpol,ncoeff,typeX,coeffs,leaks,nleaks) :
    """ Miriad wrapper - see miriad documentation for full description
        input :
            baseline -
            maxpol -
    ...
    nleaksX = miriad_io.copy_intp(nleaks)
    try :
        XX = mx.safecall(miriad_io.moduvlkcorr_,(baselineX,maxpolX,ncoeffX,typeX,coeffsX,leaksX,nleaksX))
        del XX
    except :
        pass
    ncoeffR = miriad_io.intp_value(ncoeffX)
...
def moduvdatopn() :
    ...
    result = None
    #try :
    result = mx.safecall(miriad_io.moduvdatopn_,(tnoX,))
    #except :
    #   pass
    tno = miriad_io.intp_value(tnoX)
    return castLogical(result),tno
...
def moduvdatrd(n) :
    ...
    try :
        XX = mx.safecall(miriad_io.moduvdatrd_,(preambleX,dataX,flagsX,nX,nreadX))
        del XX
    except :
        pass
...
def moduvpolinp(maxpol) :
    ...
    polsX = intArray(maxpol)
    try :
        XX = mx.safecall(miriad_io.moduvpolinp_,(maxpolX,npolX,polsX))
        del XX
    except :
        pass
    npolR = miriad_io.intp_value(npolX)
...
def moduvdatinp(inp,startch,endch,flags) :
    """ Miriad wrapper - see miriad documentation for full description
        input :
            key -
            flags -
        returns :   (as a tuple)
    """
    startchX = miriad_io.copy_floatp(startch)
    endchX = miriad_io.copy_floatp(endch)
    try :
        XX = mx.safecall(miriad_io.moduvdatinp_,(inp,startchX,endchX,flags,len(key),len(flags)))
        del XX
    except :
        pass

def moduvdatpy(vis1,vis2,ltype=" ",nchan=0,lstart=1.0,lwidth=1.0,lstep=1.0,lflags=1.0,refline=" ",rstart=1.0,rwidth=1.0,flags="") :
    """ Miriad wrapper for uvdatinp routine specifically adapted for python
        input :
            vis1,vis2 - the input visibility files
            ltype,nchan,lstart,lwidth,lflags - line parameters
    ...
    rwidthX = miriad_io.copy_floatp(rwidth)
    try :
        XX = mx.safecall(miriad_io.moduvdatpy_,(vis1,vis2,ltype,nchanX,lstartX,lwidthX,lstepX,lflagsX,refline,rstartX,rwidthX,flags,len(vis1),len(vis2),len(ltype),len(refline),len(flags)))
    except :
        pass
    return
