...
__date__ = "$Date: 2009/04/01 19:15:50 $"
...
def makeRequests(callable_, args_list, callback=None,
        exc_callback=_handle_thread_exception,result=None):
        ...
        if isinstance(item, tuple):
            requests.append(
                WorkRequest(callable_, item[0], item[1], callback=callback,
                    exc_callback=exc_callback,result=result)
            )
        else:
            requests.append(
...
class WorkRequest:
    ...
    def __init__(self, callable_, args=None, kwds=None, requestID=None,
            callback=None, exc_callback=_handle_thread_exception,result=None):
